...
export const APP_SCREENS: readonly { path: string; describes: string }[] = [
  ...
  {
    path: '/app/employees/import',
    describes: 'Bulk-import employees from a CSV/TSV/xlsx file with AI-assisted column mapping (create and update)',
  },
  ...
  // /app/me/profile is held back from the first public build (see nav-items.ts) and the
  // route returns 404, so it stays out of the catalogue — the rule above is that a path
  // Missy can offer must be a path that loads.
